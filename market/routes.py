    ...
    if request.method=='POST':
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"Unfortunately! you don't have authority to sell the {p_item_object.name}",category='danger')
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Account created successfully! You are not logged in as {user_to_create.user_name}',category='success')
        return redirect(url_for('market_page'));
